-- MES: allocate one knitting production piece across multiple garments
begin;

create table if not exists public.mes_knitting_production_allocations (
  production_id uuid not null references public.mes_knitting_production(id) on delete cascade,
  garment_id uuid not null references public.mes_garments(id) on delete cascade,
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()),
  primary key (production_id, garment_id)
);

create index if not exists idx_mes_knitting_allocations_garment
  on public.mes_knitting_production_allocations(garment_id);

drop trigger if exists trg_mes_knitting_allocations_updated_at
  on public.mes_knitting_production_allocations;
create trigger trg_mes_knitting_allocations_updated_at
before update on public.mes_knitting_production_allocations
for each row execute procedure public.set_updated_at();

alter table public.mes_knitting_production_allocations enable row level security;

drop policy if exists "mes_knitting_allocations_read"
  on public.mes_knitting_production_allocations;
create policy "mes_knitting_allocations_read"
  on public.mes_knitting_production_allocations
  for select to authenticated
  using (true);

drop policy if exists "mes_knitting_allocations_insert"
  on public.mes_knitting_production_allocations;
create policy "mes_knitting_allocations_insert"
  on public.mes_knitting_production_allocations
  for insert to authenticated
  with check (true);

drop policy if exists "mes_knitting_allocations_update"
  on public.mes_knitting_production_allocations;
create policy "mes_knitting_allocations_update"
  on public.mes_knitting_production_allocations
  for update to authenticated
  using (true)
  with check (true);

drop policy if exists "mes_knitting_allocations_delete"
  on public.mes_knitting_production_allocations;
create policy "mes_knitting_allocations_delete"
  on public.mes_knitting_production_allocations
  for delete to authenticated
  using (true);

insert into public.mes_knitting_production_allocations (production_id, garment_id)
select id, garment_id
from public.mes_knitting_production
where garment_id is not null
on conflict (production_id, garment_id) do nothing;
...
knitting_allocation_counts as (
  select
    production_id,
    greatest(count(*), 1)::numeric as allocation_count
  from public.mes_knitting_production_allocations
  group by production_id
),
knitting as (
  select
    coalesce(a.garment_id, kp.garment_id) as garment_id,
    sum(
      coalesce(kp.amount_pen, 0)
      / nullif(coalesce(kac.allocation_count, 1), 0)
      / nullif(g.effective_exchange_rate, 0)
    ) as tejido
  from public.mes_knitting_production kp
  left join public.mes_knitting_production_allocations a on a.production_id = kp.id
  left join knitting_allocation_counts kac on kac.production_id = kp.id
  join garments_with_rate g on g.id = coalesce(a.garment_id, kp.garment_id)
  where kp.piece_status = 'Terminado'
  group by coalesce(a.garment_id, kp.garment_id)
)
select
  g.id,
  g.style_number,
...
grant select, insert, update, delete on public.mes_knitting_production_allocations to authenticated;
